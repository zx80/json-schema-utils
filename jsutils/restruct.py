from .utils import JsonSchema, SchemaPath, Jsonable, log
...
# iterative type upward propagation
def updateTypes(schema: JsonSchema, path: SchemaPath):
    ...
    if types != init_types:
        # we have a change, so we need to keep on iterating
...
def restructFlt(schema: JsonSchema, path: SchemaPath) -> bool:
    if isinstance(schema, dict):
        used = used_props(schema, "type", "const", "enum", "allOf", "anyOf", "oneOf")
        if len(used) > 1:
            # TODO should keep only one?
            pass
    return True

def restructure(schema: JsonSchema):
    """Restructure a JSON Schema to match JSON Model converter constraints."""
    return recurseSchema(schema, ".", restructFlt, noRwt)
